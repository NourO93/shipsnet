...
# bring in CORs for cross-domain requests (between 2 local servers)
cors = CORS(app)
app.config['CORS_HEADERS'] = 'Content-Type'

# load keras model & graph
...
# bring in scan image from pyimagesearch (helper modules not my code, from Adrian Rosebrock
...
# initialize flask api w/ POST endpoint
@app.route('/predict', methods=["POST"])
@cross_origin()
def simple():
    print(request, file=sys.stdout)
    try:
        # bring in png data from leaflet & convert to png
        data = request.form
        bit64 = data['data']
        tag = data['tag']
    ...
    #scan image w/ slider window
    result = scan_image(model,im,11,.6)


    #export plot as image and send back in response
    fig=Figure()
    ...
    plt.savefig('images/buf-save'+tag+'.png',transparent=True, format='png')
